...
export const HistoryTable: React.FC<HistoryTableProps> = ({ history, title }) => {
  ...
  const totalProfit = sortedHistory.reduce((sum, t) => sum + t.profit, 0);

  return (
    <div className="space-y-4">
        <div className="flex justify-between items-end px-1">
             <h3 className="text-lg font-bold text-slate-200">{title}</h3>
             <div className={`px-3 py-1 rounded-lg border text-sm font-mono font-medium flex items-center gap-2 ${
                 totalProfit >= 0 ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-400'
             }`}>
            ...
            <tbody className="divide-y divide-slate-700/50">
                ...
                <tr key={trade.id} className="hover:bg-slate-700/20 transition-colors group">
                    <td className="px-4 py-3 text-slate-400 text-xs">
                        <div><span className="text-slate-500">Close:</span> {trade.closeTime ? new Date(trade.closeTime).toLocaleTimeString() : '-'}</div>
                        <div className="text-[10px] opacity-60">{trade.closeTime ? new Date(trade.closeTime).toLocaleDateString() : '-'}</div>
                    </td>
                    <td className="px-4 py-3 font-medium text-slate-200">
                         {trade.symbol}
                         <div className="text-[10px] text-slate-500 font-mono">#{trade.ticket}</div>
                    </td>
                    <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                        <span className={`p-1 rounded ${
                            trade.type === TradeType.BUY ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                        }`}>
                            {trade.type === TradeType.BUY ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                        </span>
